...
//process attributes ---- may change later
struct process {
    int pid;
    int ppid;
    ...
    int interest;
    int occupied;
    int being_waited_upon;
    int assigned;
    struct semaphore* waitpid_exit_sem;
    struct semaphore* menu_sem;
};

//process table
extern struct process process_table[150]; //might need to change value
int numthreads;
...
struct thread {
    ...
    //keep track of pid and ppid so it's consistent across threads and processes
    int pid;
    int ppid;
};
