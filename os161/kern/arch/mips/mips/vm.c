...
vm_fault(int faulttype, vaddr_t faultaddress) {
    ...
    if (faultaddress >= vbase_data && faultaddress < vtop_data) {
        ...
        if (paddr == 0) { //sbrk sets the bounds, vmfault+getppages allocate --- one page only
            ...
            result = vfs_open(as->program_name, 0, &v); // 0 for read only
            if (result) {//should i close the file here?
                vfs_close(v); //////////////////////////////
                return result;
            }
            //                load_segment(v, );
//            kprintf("I am before load sement \n ");
            result = load_segment(v, (as->data.offset_attribute + faultaddress - vbase_data), PADDR_TO_KVADDR(paddr), PAGE_SIZE, (as->data.file_size_attribute - faultaddress + vbase_data), 0);
//            kprintf("I am after load sement \n ");
            if (result) {
                vfs_close(v);
                return result;
            }
        ...
        if (paddr == 0) { //sbrk sets the bounds, vmfault+getppages allocate --- one page only
            ...
            result = vfs_open(as->program_name, 0, &v);
            if (result) {
                vfs_close(v); //
                return result;
            }
            //                load_segment(v, );
            result = load_segment(v, (as->text.offset_attribute + faultaddress - vbase_text), PADDR_TO_KVADDR(paddr), PAGE_SIZE, (as->text.file_size_attribute - faultaddress + vbase_text), 0);
            if (result) {
                vfs_close(v);
                return result;
            }
