...
//assignment 2 stuff
//struct lock* global_process_lock;
//struct process process_table[65];
struct process process_table[150]; //might ;
...
void create_process(struct thread* current_thread) {
    int pid = find_next_empty();

    //stuff that's in the process table
    process_table[pid].pid = pid;
    process_table[pid].thread_ptr = current_thread;
    process_table[pid].exited = 1;
    process_table[pid].exitcode = 0;
    process_table[pid].interest = 0;
    process_table[pid].being_waited_upon = 0;
    process_table[pid].assigned = 1;

    //stuff that's in the thread
    current_thread->pid = pid;
    current_thread->ppid = 0;
    numthreads++;               // do it here or thread create???
    
    //create sem only for menu ---- which is the first thread
    if (pid == 1) {
        process_table[1].menu_sem = sem_create("menu_sem", 0);
    }
}

//find the next empty pid
int find_next_empty() {
    int i;
    for (i = 1; i <= 149; i++) {
        if (process_table[i].thread_ptr == NULL && process_table[i].assigned == 0) //change the conditions according to struct
            return i;
    }
    return 0;
}
...
void initialize_process_table() {
    ...
    for (i = 0; i <= 149; i++) {
        ...
        process_table[i].thread_ptr = NULL;
        process_table[i].exited = 0; //0 = exited   1 = not exited
        process_table[i].exitcode = 0;
        process_table[i].interest = 0;
        process_table[i].being_waited_upon = 0;
        process_table[i].assigned = 0;
        process_table[i].waitpid_exit_sem = sem_create("sem", 0);
    }
}
...
thread_bootstrap(void) {
    ...
    //initialize all the global process tables
...
thread_fork(const char *name,
    ...
    //ppid update to ensure synch between user and kernel space
    newguy->ppid = curthread->pid;
    process_table[newguy->pid].ppid = newguy->ppid;
...
thread_exit(void) {
    ...
    assert(numthreads > 0); //maybe reset to NULL
    numthreads--;

    //wake up menu thread is it's the current thread
    if (curthread->ppid == 1) {
        V(process_table[1].menu_sem);
    }
