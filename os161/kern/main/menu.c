...
common_prog(int nargs, char **args) {
    int result;
    //create a "menu thread"
    //    struct thread* menuthread = thread_create("");
#if OPT_SYNCHPROBS
    ...
    //first thread aka menu thread sleep after forking so child can run
    if (curthread->pid == 1) {
        P(process_table[1].menu_sem);
    }
...
static int cmd_dbflags(int n, char **a) {
    (void) n;
    (void) a;
...
static int cmd_df(int n, char **a) {
    //kprintf("number of args %d\n", n);
    //kprintf("cmd is   %s\n", a[0]);
    //kprintf("num is   %s\n", a[1]);
    //kprintf("arg 3 is   %s\n", a[2]);
    //kprintf("dbflag is currently %d\n", dbflags);

    u_int32_t flag = 1;
    int count = atoi(a[1]);
    int i = 1;

    if (count <= 12 && count >= 1 && n == 3) {
        while (i < count) {
            flag = flag << 1;
            i++;
        }
