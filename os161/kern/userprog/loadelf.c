...
load_elf(struct vnode *v, vaddr_t *entrypoint)
{
	...
	for (i=0; i<eh.e_phnum; i++) {
		...
		result = as_define_region(curthread->t_vmspace,
					  ...
					  ph.p_flags & PF_W,
					  ph.p_flags & PF_X, 
                                          ph.p_offset, ph.p_filesz);//modifed
		if (result) {
			return result;
		}
	}

	/*  this section is commented out because we are doing demand paging (loading specified segments) in vm_fault*/
